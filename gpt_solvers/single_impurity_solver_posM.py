...
def self_cons_equations_single(N, M, x, y, z, theta):
    ...
    pref_num = z * c - y * M * (c ** 2 + s ** 2)
    if chi < 1e-12:
        # Лимит [arccot(a+chi)-arccot(a-chi)] / chi -> -2/(1+a^2)
        m = (-2.0 / np.pi) * pref_num / (1.0 + a * a)
    else:
        prefactor = pref_num / chi
        m = (1.0 / np.pi) * (arccot(a + chi) - arccot(a - chi)) * prefactor
...
def _clip_nm_posM(nm):
    N, M = np.asarray(nm, dtype=float)
    return np.array([np.clip(N, 0.0, 2.0), np.clip(M, 0.0, 1.0)], dtype=float)


def _clip_xm_posM(xm, x_bounds):
    x, M = np.asarray(xm, dtype=float)
    xl, xr = x_bounds
    return np.array([np.clip(x, xl, xr), np.clip(M, 0.0, 1.0)], dtype=float)


def projected_picard_nm_posM(nm0, x, y, z, theta, damping=0.65, n_steps=3):
    nm = _clip_nm_posM(nm0)
    for _ in range(n_steps):
        rhs = np.array(self_cons_equations_single(nm[0], nm[1], x, y, z, theta), dtype=float)
        nm = _clip_nm_posM((1.0 - damping) * nm + damping * rhs)
    return nm
...
def generate_nm_seeds_posM(x, y, z, theta,
                           N_grid=7, M_grid=5,
                           n_halton=24,
                           picard_steps=3,
                           damping=0.65,
                           max_start_points=24):
    grid_pts = _coarse_grid((0.0, 2.0), (0.0, 1.0), N_grid, M_grid)
    ...
    scored = []
    for s in seeds:
        sr = projected_picard_nm_posM(s, x, y, z, theta, damping=damping, n_steps=picard_steps)
...
def generate_xm_seeds_posM(N_fixed, y, z, theta,
                           x_bounds=(-20.0, 20.0),
                           x_grid=9, M_grid=5,
                           n_halton=32,
                           max_start_points=32):
    xl, xr = map(float, x_bounds)
    grid_pts = _coarse_grid((xl, xr), (0.0, 1.0), x_grid, M_grid)
    halton = _halton_points(n_halton, dim=2) if n_halton > 0 else np.empty((0, 2), dtype=float)
    if len(halton):
    ...
    special = np.array([
        [xl, 0.0], [xl, 0.5], [xl, 1.0],
        [0.5 * (xl + xr), 0.0], [0.5 * (xl + xr), 0.5], [0.5 * (xl + xr), 1.0],
        [xr, 0.0], [xr, 0.5], [xr, 1.0],
    ], dtype=float)

    seeds = np.vstack([special, grid_pts, halton])
    scored = []
    for s in seeds:
        s = _clip_xm_posM(s, x_bounds)
...
def validate_nm_solution_posM(sol, x, y, z, theta, residual_tol=1e-9, bound_tol=1e-10):
    sol = np.asarray(sol, dtype=float)
    N, M = sol
    if not (0.0 - bound_tol <= N <= 2.0 + bound_tol and 0.0 - bound_tol <= M <= 1.0 + bound_tol):
        return False, np.inf
...
def validate_xm_solution_posM(sol, N_fixed, y, z, theta, x_bounds=(-20.0, 20.0), residual_tol=1e-9, bound_tol=1e-10):
    sol = np.asarray(sol, dtype=float)
    x, M = sol
    xl, xr = x_bounds
    if not (xl - bound_tol <= x <= xr + bound_tol and 0.0 - bound_tol <= M <= 1.0 + bound_tol and 0.0 - bound_tol <= N_fixed <= 2.0 + bound_tol):
        return False, np.inf
...
def self_cons_single_posM(x, y, z, theta,
                          N_grid=7,
                          M_grid=5,
                          n_halton=24,
                          picard_steps=3,
    ...
    с дополнительным ограничением M >= 0.

    Возвращает список решений:
        [np.array([N, M]), ...]
    с ограничениями 0 <= N <= 2, 0 <= M <= 1.
    """
    seeds = generate_nm_seeds_posM(x, y, z, theta,
    ...
    for seed in seeds:
        ...
        sol = _clip_nm_posM(ans.x)
        ok, res_inf = validate_nm_solution_posM(sol, x, y, z, theta, residual_tol=residual_tol)
        item = {
...
def self_cons_single_fixed_N_posM(N, y, z, theta,
                                  x_bounds=(-20.0, 20.0),
                                  x_grid=9,
                                  M_grid=5,
                                  n_halton=32,
    ...
    for seed in seeds:
        ...
        sol = _clip_xm_posM(ans.x, x_bounds)
        ok, res_inf = validate_xm_solution_posM(sol, N, y, z, theta, x_bounds=x_bounds, residual_tol=residual_tol)
        item = {
            'success': bool(ans.success),
            'status': int(ans.status),
            'message': ans.message,
